...
class TestFetchHackernewsTop:
    ...
    @pytest.mark.asyncio
    async def test_returns_formatted_html(self) -> None:
        mock_stories = [
            {
                "id": 1,
                "title": "Story 1",
        ...
        async def get_side_effect(url, **kwargs):
            ...
            for story in mock_stories:
                if str(story["id"]) in url:
                    ...
                    return resp
            resp = MagicMock()
            resp.raise_for_status.side_effect = RuntimeError("not found")
            return resp

        client = MagicMock()
        client.get = AsyncMock(side_effect=get_side_effect)

        with patch("tele_home_supervisor.scheduled._get_client", return_value=client):
            result = await scheduled.fetch_hackernews_top(limit=2)
    ...
    async def test_handles_api_error(self) -> None:
        ...
        client.get = AsyncMock(
            side_effect=real_requests.RequestException("Network error")
        )

        with patch("tele_home_supervisor.scheduled._get_client", return_value=client):
            result = await scheduled.fetch_hackernews_top(limit=3)
...
class TestFetchEpicFreeGames:
    ...
    async def test_returns_tuple(self) -> None:
        with patch("tele_home_supervisor.scheduled._cached_fetch") as mock_cache:
            ...
            result = await scheduled.fetch_epic_free_games()
...
class TestFetchSteamFreeGames:
    ...
    async def test_returns_tuple(self) -> None:
        with patch("tele_home_supervisor.scheduled._cached_fetch") as mock_cache:
            ...
            result = await scheduled.fetch_steam_free_games(limit=5)
...
class TestFetchGogFreeGames:
    ...
    async def test_returns_tuple(self) -> None:
        with patch("tele_home_supervisor.scheduled._cached_fetch") as mock_cache:
            ...
            result = await scheduled.fetch_gog_free_games()
...
class TestFetchHumbleFreeGames:
    ...
    async def test_returns_tuple(self) -> None:
        with patch("tele_home_supervisor.scheduled._cached_fetch") as mock_cache:
            ...
            result = await scheduled.fetch_humble_free_games()
...
class TestBuildCombinedGameOffers:
    ...
    async def test_combines_all_sources(self) -> None:
        with (
            ...
            result, error = await scheduled.build_combined_game_offers()
...
class TestCachedFetch:
    ...
    async def test_caches_result(self) -> None:
        ...
        result1 = await scheduled._cached_fetch("test_key", ttl_s=3600, fetcher=fetcher)
        result2 = await scheduled._cached_fetch("test_key", ttl_s=3600, fetcher=fetcher)
